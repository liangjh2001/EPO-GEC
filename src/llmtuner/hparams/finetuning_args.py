...
class RLHFArguments:
    ...
    pref_margin: float = field(
        default=0.0,
        metadata={"help": "The margin parameter in the preference loss."},
    )
    ...
    use_edit_mask: bool = field(
        default=False,
        metadata={"help": "Whether or not to use edit mask."},
    )
    use_length_normalized: bool = field(
        default=False,
        metadata={"help": "Whether or not to use length normalized."},
    )
    pivot_token_weight: float = field(
        default=5.0
    )
    edit_token_weight: float = field(
        default=1.0
    )
    normal_token_weight: float = field(
        default=0.0
    )
...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments):
    ...
    pair_revise_alpha: float = field(
        default=0.5
    )
    pair_revise_beta: float = field(
        default=0.5
    )
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "orpo",
    "aux_gec", "echo_gec", "pair_revise", "custom_dpo", "custom_dpo_pair"] = field(
        default="sft",
    ...
    def __post_init__(self):
        ...
        self.use_ref_model = (self.stage == "dpo" or self.stage == "custom_dpo" or self.stage == "custom_dpo_pair") \
                             and self.pref_loss not in ["orpo","simpo"]
