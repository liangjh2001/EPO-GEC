...
def get_batch_logps_with_edit_mask(
        logits: "torch.Tensor",
        labels: "torch.Tensor",
        edit_masks: "torch.Tensor",
        pivot_token_weight: float = 5.0,
        edit_token_weight: float = 1.0,
        normal_token_weight: float = 0.0,
    ...
    Returns:
        logps: A tensor of shape (batch_size,) containing the sum of log probabilities.
        valid_length: A tensor of shape (batch_size,) containing the number of non-masked tokens.
        logps_with_edit_mask: A tensor of shape (batch_size,) containing the sum of log probabilities with edit mask considered.
        valid_length_with_edit_mask: A tensor of shape (batch_size,) containing the number of non-masked tokens with edit mask considered.
    """
    if logits.shape[:-1] != labels.shape:
        raise ValueError("Logits (batchsize x seqlen) and labels must have the same shape.")
    ...
    edit_mask_adjusted = torch.where(
        edit_masks[:, :-1] == 1,
        pivot_token_weight,
        torch.where(
            edit_masks[:, :-1] == 2,
    ...
    logps_with_edit_mask = (per_token_logps * edit_mask_adjusted * loss_mask).sum(-1)

    logps = (per_token_logps * loss_mask).sum(-1)
    valid_length = loss_mask.sum(-1)

    combined_mask = loss_mask & (edit_masks[:, :-1] == 1)
    valid_length_with_edit_mask = combined_mask.sum(-1)

    return logps, valid_length, logps_with_edit_mask, valid_length_with_edit_mask
