...
from ChERRANT.modules.alignment import read_cilin, read_confusion, AlignmentNoPos
...
def process_item(data):
    ...
    for token_id in tokens_ids2:
        converted_sentence2.append(tokenizer.decode(token_id))
    #converted_sentence2 = tokenizer.convert_ids_to_tokens(tokens_ids2)
    for p in predict:
        converted_sentence1 = []
        tokens_ids1 = tokenizer(p, add_special_tokens=False).input_ids
        for token_id in tokens_ids1:
            converted_sentence1.append(tokenizer.decode(token_id))
        #converted_sentence1 = tokenizer.convert_ids_to_tokens(tokens_ids1)

        result = []
        for converted_sentence in [converted_sentence1, converted_sentence2]:
